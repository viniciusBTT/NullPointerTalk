...
- A presença faz chamadas servidor→servidor no LiveKit, e elas vão direto em **loopback** via `LIVEKIT_API_URL` (definido no `docker-compose.yml`, default `http://localhost:7880`). Sem isso o backend derivaria de `livekit.url` e daria a volta pelo DNS público + nginx + TLS a cada 1,5s pra falar com o processo vizinho — e a presença cairia junto se o certificado vencesse. Fica no compose, e **não** no `application-vps.properties`, porque a cópia daquele arquivo na VPS tem as credenciais reais e divergiu do repositório: um commit que o toque quebra o `git pull` do deploy.
- Com o `LIVEKIT_API_URL` em loopback, dá pra **negar `/twirp/` no nginx** (`location /twirp/ { deny all; }`) — senão a API de administração do LiveKit fica alcançável da internet. **A ordem importa**: o loopback precisa estar valendo antes de negar, senão a presença para.
